 ...
 */
class GetAvatar  @Inject
internal constructor(val repository: Repository, threadExecutor: ThreadExecutor,
                     postExecutionThread: PostExecutionThread) : UseCase<AvatarTemplate, GetAvatar.Params>(threadExecutor, postExecutionThread) {

    override fun buildUseCaseObservable(params: Params): Observable<AvatarTemplate> {
        return repository.getAvatar(params.userName,params.authToken)
    }

    class Params(val userName: String, val authToken: String)
